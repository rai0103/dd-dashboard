...
// （S&P500指数そのもの＝SPXシンボルはTwelve Data無料プランでは利用不可のため、
//   ダッシュボード側の既存設計（S&P500系列＝SPY終値をそのまま使う）に合わせ、sp500にはSPY終値を用いる）
// 応答: { date: "YYYY-MM-DD", sp500: number, spy: number, voo: number }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      return jsonResponse({ date: spy.date, sp500: spy.close, spy: spy.close, voo: voo.close });
    } catch (e) {
