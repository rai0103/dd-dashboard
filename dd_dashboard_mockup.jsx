  ...
  return (
    <FullScreenModal title="データの入力" onClose={onClose}>
      ...
      <div className="mt-3 text-[10px] leading-relaxed" style={{ color: C.textDim }}>
        入力したデータはこの環境の永続ストレージ（window.storage・非共有）に保存され、次回もこの内容で開始します。実運用でGitHub Pagesに公開する際はブラウザのIndexedDBに置き換える想定です。
      </div>
    </FullScreenModal>
  );
}
...
export default function DDDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("voo_price_history", false);
        if (res && res.value) {
      ...
      try {
        const res2 = await window.storage.get("portfolio_holdings", false);
      ...
      try {
        const res3 = await window.storage.get("classification_overrides", false);
  ...
  async function persist(series) {
    try { await window.storage.set("voo_price_history", JSON.stringify(series.map((p) => ({ date: p.date.toISOString().slice(0, 10), price: p.price }))), false); } catch (e) { /* storage unavailable */ }
  }
  async function persistHoldings(list) {
    try { await window.storage.set("portfolio_holdings", JSON.stringify(list), false); } catch (e) { /* storage unavailable */ }
  }
  async function persistOverrides(map) {
    try { await window.storage.set("classification_overrides", JSON.stringify(map), false); } catch (e) { /* storage unavailable */ }
  }
  ...
  function handleReset() { setRawSeries(SEED_SERIES); setDataSource("seed"); window.storage.delete("voo_price_history", false).catch(() => {}); }
  ...
  function handleResetHoldings() { setHoldings(HOLDINGS_DEFAULT); setHoldingsSource("seed"); window.storage.delete("portfolio_holdings", false).catch(() => {}); }
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ background: C.bg, color: C.text, minHeight: 820, fontFamily: "'Zen Kaku Gothic New','Hiragino Kaku Gothic ProN',sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Zen+Kaku+Gothic+New:wght@400;500;700&family=JetBrains+Mono:wght@400;500;600&display=swap');
